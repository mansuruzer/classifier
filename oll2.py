...
sınıflandıran script. Ara-kayıt (checkpoint), satır aralığı belirleme (paralel çalışma)
ve devam etme (resume) özellikli.
"""
...
# 1. Bilgisayar: START_ROW = 0,     END_ROW = 8000
# 2. Bilgisayar: START_ROW = 8000,  END_ROW = 18729
# 3. Bilgisayar: START_ROW = 18729, END_ROW = 29458

START_ROW = 0        # Başlangıç satır indeksi (0'dan başlar)
END_ROW = 8000       # Bitiş satır indeksi (Bu satıra kadar işler, dahil değil)
# -----------------------------------------------------------------

# ------------------- DOSYA VE GENEL AYARLAR -------------------
CSV_DOSYASI = "converted_43.csv"
OUTPUT_DOSYASI = f"sonuc_arizalar_{START_ROW}_{END_ROW}.csv"  # Her bilgisayar kendi aralığını kaydeder
...
TEST_MODU = False        # True: Sadece TEST_SATIR_SAYISI kadar satır işler
TEST_SATIR_SAYISI = 100
KAYIT_ARALIGI = 100      # Her N satırda bir diske yaz
TIMEOUT_SANIYE = 60
MAX_DENEME = 3
# --------------------------------------------------------------
...
def main():
    if os.path.exists(OUTPUT_DOSYASI):
        ...
        full_df = csv_oku(CSV_DOSYASI)
        # Belirtilen aralığı dilimle (slice)
    ...
    print(f"İşlem Aralığı: {START_ROW} - {END_ROW} (Toplam {len(df)} satır)")
    ...
    for idx in tqdm(calisilacak, total=len(calisilacak)):
        ...
        if zaten_var:
            ...
            if mevcut_label in ("MEK", "MEKANİK"):
                df.at[idx, KATEGORI_KOLONU] = "MEK"
                continue
            if mevcut_label in ("OTO", "OTOMASYON"):
                df.at[idx, KATEGORI_KOLONU] = "OTO"
                continue
